...
const Card = ({sectionTitle, cardTitle, id, imgId}) => {
  
    const {store, actions} = useContext(Context);

    
    const [isActive, setIsActive] = useState(store.favorites.some(x => x.id == id))

    useEffect(() => {
        if (store.favorites.find(x => x.id == id) == undefined) setIsActive(false);
    }, [store.favorites])
    ...
    const setIsActiveHandler = () => {
        if (!isActive) {
            ...
            actions.removeFavorite2(cardTitle)
            setIsActive(false)
        }
    ...
    return (
        <div className="card col-5" style={{width: "18rem"}}>
            ...
            <div className="card-body">
                ...
                <button name={id} className={`btn btn-outline-warning ms-auto ${isActive ? "active" : ""}`} onClick={setIsActiveHandler}>
